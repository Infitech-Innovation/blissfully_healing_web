import { RetreatDetails } from "@/types/retreats.definations";
import {
    ArrowLeft,
    CalendarDays,
    Check,
...
export function RetreatDetailView({ retreat }: RetreatDetailViewProps) {
    const detailStats = [
        { icon: Clock, label: "Duration", value: retreat.duration },
        {
            icon: Users,
            label: "Group Size",
            value: `${retreat.min_guests} - ${retreat.max_guests} Guests`
        },
        { icon: MapPin, label: "Location", value: retreat.location },
    ];

    const sortedIncluded = [...(retreat.included_items || [])].sort((a, b) => a.order - b.order);
    const sortedItinerary = [...(retreat.itinerary_items || [])].sort((a, b) => a.order - b.order);
                    ...
                    <div className="min-w-0">
                        ...
                        <section className="mt-10">
                            ...
                            <h2 className="font-serif text-4xl font-semibold text-[#2f251f] md:text-5xl">
                                {retreat.details_heading || "A Gentle Container For Return"}
                            </h2>
                            <p className="mt-5 max-w-3xl text-base leading-8 text-[#6f5c4f] whitespace-pre-line">
                                {retreat.details_text}
                            </p>
                        </section>

                        {/* Inclusions & Schedules Grid */}
                        <section className="mt-10 grid gap-6 md:grid-cols-2">
                            ...
                            <article className="rounded-[8px] border border-[#eadfd4] bg-white p-6 shadow-[0_14px_38px_rgba(63,52,44,0.07)]">
                                ...
                                <div className="flex flex-col gap-3">
                                    {sortedIncluded.map((item) => (
                                        <div key={item.id} className="flex gap-3 text-sm leading-6 text-[#6f5c4f]">
                                ...
                                <div className="flex flex-col gap-4">
                                    {sortedItinerary.map((item, index) => (
                                        <div key={item.id} className="flex gap-3">
                                            ...
                                            <div className="pt-0.5 text-sm leading-6 text-[#6f5c4f]">
                                                <p className="font-bold text-[#2f251f]">{item.title}</p>
                    ...
                    <aside className="rounded-[8px] border border-[#eadfd4] bg-white p-6 shadow-[0_18px_45px_rgba(63,52,44,0.08)] lg:sticky lg:top-6">
                        ...
                        <div className="mt-4 flex items-end justify-between gap-4 border-b border-[#eadfd4] pb-5">
                            ...
                            <div className="text-right">
                                <p className="text-2xl font-semibold text-[#2f251f]">{formattedPrice}</p>
                                {retreat.spots_remaining <= 3 && retreat.spots_remaining > 0 && (
                                    <p className="text-[11px] font-medium text-amber-600 mt-1">
                                        Only {retreat.spots_remaining} spots remaining
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <RegisterButton
                            slug={retreat.slug} price={retreat.price}
                        />
