            ...
            <div className="mx-auto max-w-7xl">
              ...
              <div className="grid gap-6 md:grid-cols-3">
                {stays?.map((stay) => (
                  <article
                    key={stay.title}
                    className="group flex flex-col overflow-hidden rounded-[8px] border border-[#eadfd4] bg-white p-4 shadow-[0_18px_45px_rgba(63,52,44,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_28px_70px_rgba(63,52,44,0.14)]"
                  >
                    ...
                    <p className="mb-4 text-sm leading-6 text-[#6f5c4f] flex-grow">
                      {stay.short_description}
                    </p>
                    <Link
                      href={`/retreats/${stay.slug}`}
                      className="flex w-full mt-auto items-center justify-center gap-2 rounded-[8px] bg-[#8f6249] px-4 py-3 text-xs font-bold uppercase tracking-widest text-white transition hover:bg-[#2f251f]"
                    >
                      View Details
                      <ArrowRight
                        size={14}
